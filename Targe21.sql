...
--- kuvab vasutpidises järjestuses
...
-- näitab ära, et mitu inimest vanemad, kui 41 ja kui palju igas linnas
select GenderId, City, sum(Age) as TotalAge, count(Id) as [Total Person(s)]
from Person
...
select City, sum(cast(Salary as int)) as TotalSalary from Employees group by City  -- ühe kuu palgafond linnade lõikes

alter table Employees
add City nvarchar(30)
select * from Department

select City, Gender, sum(cast(Salary as int)) as TotalSalary from Employees group by City, Gender -- toome soolise erisuse päringusse
...
select count(*) from Employees -- loeb ära, mitu inimest on nimekirjas, * asemele võib panna ka muid veergude nimetusi
...
select DATEPART(weekday, '2022-04-24 12:02:36.260') -- kuvab 1 kuna USA nädal algab pühapäevaga
select DATEPART(month, '2022-04-24 12:02:36.260')  --kuvab kuu nr
select DATEADD(DAY, 20, '2022-04-24 12:02:36.260') -- liidab stringis olevale kp 20 päeva juurde
select DATEADD(DAY, -20, '2022-04-24 12:02:36.260')  -- lahutab 20 päeva maha
...
---
-- 1.Vaikimisi primaarvõti loob unikaalse klastris oleva indksi, samas unikaalne piirang
...
-- Esimene on parameeter on veateate sisu, teine on veataseme nr (nr 16 tähendab üldiseid vigu),
-- kolmas on olek
...
--kustutada ära InsteadOfDelete trigger vEmployeeDetails alt
...
--- harjutus 48 Päritud tabelid ja CTE
...
-- kui on kaks CTE-d, siis unioni abli ühendad päringu
...
-- merge puhul peab alalti olema vähemalt kaks tabelit:
...
--- transaction taga, etmõlemad uuendatavad käsud saavad ära tehtud
...
--- kui lubada samaaegselt kõõik tehingud ära teha, siis see omakorda tekitab probleeme
...
--- iga juhtumile tuleb läheneda juhtumipõhiselt
...
-- 2. käsklus
-- samal tegin uu päringuga akna, kus kohe peale 
-- peale esimest käivitan teise
--- 2  transaction
